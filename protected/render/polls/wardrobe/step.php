            ...
            <section id="content">
                <div class="container">
                    <?  APP::Render('core/widgets/template/header', 'include', [
                            $data['poll'] => 'polls/wardrobe/' . APP::Module('Routing')->get['token'],
                            'user_menu' => false
                        ]) ?>
                            ...
                            <form method="post" id="step<?= $data['step'] ?>">
                                ...
                                switch ($data['step']) {
                                    case 1:
                                        ...
                                        Перечислите 5 самых больших проблем с гардеробом, которые у вас возникают при наступлении весенне-летнего сезона.
                                        <br><br>
                                        ...
                                        <textarea class="form-control" name="answers[44]" placeholder="Напишите Ваш ответ"></textarea>
                                        <?
                                        break;
                                    case 2:
                                        ?>
                                        <input type="hidden" name="step" value="2">
                                        Какие более глубокие проблемы вы бы хотели решить, меняя свой гардероб? Или чего вы хотите достичь с помощью перемен во внешнем виде?
                                        <br><br>
                                        <textarea class="form-control" name="answers[45]" placeholder="Напишите Ваш ответ"></textarea>
                                        <?
                                        break;
                                }
                                ?>
        ...
        <script>
            $(document).ready(function() {
                ...
                switch ($data['step']) {
                    case 1:
                        ?>
                        autosize($('[name="answers[44]"]'));
                        <?
                        break;
                    case 2:
                        ?>
                        autosize($('[name="answers[45]"]'));
                        <?
                        break;
                }
                ?>
                ...
                $('#step1').submit(function() {
                    if ($('[name="answers[44]"]').val() === '') { 
                        swal({
                            title: 'Перечислите 5 самых больших проблем с гардеробом, которые у вас возникают при наступлении весенне-летнего сезона.',
                            type: 'error',
                ...
                $('#step2').submit(function() {
                    if ($('[name="answers[45]"]').val() === '') { 
                        swal({
                            title: 'Какие более глубокие проблемы вы бы хотели решить, меняя свой гардероб?',
                            type: 'error',
                            showConfirmButton: true,
                            allowOutsideClick: true
                        });

...
APP::$insert['js_mail_letter_selector'] = ['js', 'file', 'before', '</body>', APP::Module('Routing')->root . 'public/modules/mail/letter-selector/script.js'];

APP::$insert['css_mail_letter_selector'] = ['css', 'file', 'before', '</body>', APP::Module('Routing')->root . 'public/modules/mail/letter-selector/style.css'];
ob_start();
?>
<script>
...
?>
<script>
    $('#mail-log-letter').MailingLetterSelector({
    ...
    function GetMailLog(nav) {
        ...
        $.ajax({
            ...
            data: {
                date: {
                    from: $('#mail-log-date-from').val(),
                    to: $('#mail-log-date-to').val()
                },
                letter: $('#mail-log-letter').val(),
                sender: null
            },
            type: 'POST',
            dataType: 'json',
    ...
    $(document).on('click', "#mail-log-period > button",function() {
        ...
        $('#mail-log-date-to').val(to);
        $('#mail-log-date-from').val(from);
    ...
    $('#mail-log-calendar').popover({
        ...
        var to_date = new Date(parseInt($('#mail-log-date-to').val()) * 1000);
        var from_date = new Date(parseInt($('#mail-log-date-from').val()) * 1000);
        ...
        $('#mail-list-calendar-from-block').on('dp.change', function(e) {
            $('#mail-log-date-from').val(Math.round(e.date._d.getTime() / 1000));
            $('#mail-log-period > button').removeAttr('disabled');
        ...
        $('#mail-list-calendar-to-block').on('dp.change', function(e) {
            $('#mail-log-date-to').val(Math.round(e.date._d.getTime() / 1000));
            $('#mail-log-period > button').removeAttr('disabled');

...
if ($comments['total']) {
    ...
    <div class="<?= isset($data['class']['holder']) ? $data['class']['holder'] : 'card' ?>">
        ...
        <div id="comments-holder" class="card-body card-padding">
            <?
            if (!function_exists('PrintComment')) {
                function PrintComment($comment, $offset = 0, $likes = true, $reply = true) {
            ...
            if (!function_exists('ChildrenComments')) {
                function ChildrenComments($comment, $children_comments, $offset = 0, $likes = true, $reply = true) {
                    $offset = $offset + 35;
                    ?>
                    <div class="children m-t-15">
                        ...
                        foreach ($children_comments as $children_comment) {
                            if ($children_comment['sub_id'] == $comment['id']) {
                                PrintComment($children_comment, $offset, $likes, $reply);
                                ChildrenComments($children_comment, $children_comments, $offset, $likes, $reply);
                            }
                        }
                        ?>
                    </div>
            ...
            foreach ($comments['root'] as $sub_id) {
                PrintComment($sub_id, 0, isset($data['likes']) ? $data['likes'] : false, isset($data['reply']) ? $data['reply'] : true);
                ChildrenComments($sub_id, $comments['children'], 0, isset($data['likes']) ? $data['likes'] : false, isset($data['reply']) ? $data['reply'] : true);
            }
            ?>
        </div>
    </div>

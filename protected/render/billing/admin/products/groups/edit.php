...
<head>
    ...
    <title>Редактрование группы продуктов</title>
...
<body data-ma-header="teal">
    ...
    <section id="main">
        ...
        <section id="content">
            <div class="container">
                <div class="card">
                    <form id="update-product-group" class="form-horizontal" role="form">
                        <input type="hidden" name="id" value="<?= APP::Module('Routing')->get['product_group_id_hash'] ?>">

                        <div class="card-header">
                            <h2>Редактрование группы продуктов</h2>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#name').val('<?= $data['product-group']['name'] ?>');
            $('#descr').val('<?= $data['product-group']['descr'] ?>');
            $('#ignore_group').val('<?= $data['product-group']['ignore_group'] ?>');
            $('#ignore_products').val('<?= $data['product-group']['ignore_products'] ?>');
            $('#ignore_group_sell').val('<?= $data['product-group']['ignore_group_sell'] ?>');
            $('#ignore_products_sell').val('<?= $data['product-group']['ignore_products_sell'] ?>');
            $('#type_id').val('<?= $data['product-group']['type_id'] ?>');

            $('#update-product-group').submit(function(event) {
                ...
                $.ajax({
                    type: 'post',
                    url: '<?= APP::Module('Routing')->root ?>admin/billing/products/groups/api/update.json',
                    data: $(this).serialize(),
                    success: function(result) {
                        switch(result.status) {
                            case 'success':
                                swal({
                                    title: 'Готово',
                                    text: 'Группа продукта "' + name.val() + '" успешно обновлена',
                                    type: 'success',
                        ...
                        $('#update-product-group').find('[type="submit"]').html('Сохранить изменения').attr('disabled', false);
                    }
                });
            });
        });

...
<body data-ma-header="teal">
                                ...
                                <div class="col-md-10">
                                    <div id="secondary-products">
                                        <? if (count($data['product']['secondary_products'])) { ?>
                                            <?php foreach ($data['product']['secondary_products'] as $key => $target_product) {
                                                ?>
                                                <div class="row m-b-10">
                                                    <div class="col-md-6">
                                                        <select class="form-control secondary-product" id="secondary-products-<?= $key ?>" name="secondary_products[<?= $key ?>][id]" data-placeholder="Выберите продукт">
                                                            <?
                                                            foreach ($data['products_list'] as $group => $products) {
                                                                ?>
                                                                <optgroup label="<?= $group ?>">
                                                                    <?
                                                                    foreach ($products as $product) {
                                                                        ?><option value="<?= $product['id'] ?>" <? if ($product['id'] == $target_product['id']) { ?>selected<? } ?>><?= $product['name'] ?> (<?= $product['amount'] ?> руб.)</option><?
                                                                    }
                                                                    ?>
                                                                </optgroup>
                                                                <?
                                                    ...
                                                    <div class="col-md-5">
                                                        <div class="input-group">
                                                            ...
                                                            <input class="m-l-20 form-control" id="secondary-products-time-<?= $key ?>" name="secondary_products[<?= $key ?>][timeout]" placeholder="timeout" type="text" value="<?= $target_product['timeout'] ?>">
                                                        </div>
                                                    </div>
                                    ...
                                    <button id="add-secondary-product" type="button" class="btn btn-default btn-labeled fa fa-plus">Добавить продукт</button>
                                </div>
                            </div>
                            <div class="form-group">
    ...
    <script>
        $(document).ready(function() {
            $('#group_id').val('<?= $data['product']['group_id'] ?>');
            $('#alt_id').val('<?= $data['product']['alt_id'] ?>');
            $('#name').val('<?= $data['product']['name'] ?>');
            $('#description').val('<?= $data['product']['description'] ?>');
            $('#amount').val('<?= $data['product']['amount'] ?>');
            $('#ignore_group').val('<?= $data['product']['ignore_group'] ?>');
            $('#ignore_products').val('<?= $data['product']['ignore_products'] ?>');
            $('#ignore_group_sell').val('<?= $data['product']['ignore_group_sell'] ?>');
            $('#ignore_products_sell').val('<?= $data['product']['ignore_products_sell'] ?>');
            $('#related_products').val('<?= $data['product']['related_products'] ?>');
            $('.secondary-product').chosen({
                width: '100%',
                allow_single_deselect: true
            });
            $('#stop_tunnels').val('<?= $data['product']['stop_tunnels'] ?>');
            $('#state').val('<?= $data['product']['state'] ?>');
            $('#multi_sale').val('<?= $data['product']['multi_sale'] ?>');
            $('#access_link').val('<?= $data['product']['access_link'] ?>');
            $('#descr_link').val('<?= $data['product']['descr_link'] ?>');
            $('#sort_index').val('<?= $data['product']['sort_index'] ?>');
            $('#sale_notify_email').val('<?= $data['product']['sale_notify_email'] ?>');
                    ...
                    success: function(result) {
                        switch(result.status) {
                            case 'success':
                                swal({
                                    title: 'Готово',
                                    text: 'Продукт "' + name.val() + '" был успешно обновлен',
                                    type: 'success',
                        ...
                        $('#update-product').find('[type="submit"]').html('Сохранить изменения').attr('disabled', false);
                    }
                });
            });
        });

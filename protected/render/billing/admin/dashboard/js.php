...
<script>
    function GetInvoices(nav) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $.plot("#billing-chart", [
                    ...
                    { 
                        label: "Открытие доступа", 
                        data: data.range.access 
                    }
                ], {
                    series: {
                        lines: {
		...
		$("#billing-chart").bind("plothover", function (event, pos, item) {
                    ...
                    '<div class="table-responsive m-b-25">',
                        '<table id="billing-invoices-table" class="table table-hover">',
                            '<thead>',
                                '<tr>',
                                    '<th width="20%">Дата</th>',
                                    '<th width="12%">Новые</th>',
                                    '<th width="12%">В обработке</th>',
                                    '<th width="12%">Аннулированные</th>',
                                    '<th width="12%">Оплаченные</th>',
                                    '<th width="12%">Открытие доступа</th>',
                            ...
                            '<tbody>',
                                '<tr class="total">',
                                    ...
                                    '<td class="t_access"><a target="_blank" href="<?= APP::Module('Routing')->root ?>admin/billing/invoices?filters=' + data.total.access + '"></a></td>',
                ...
                var billing_invoices = [0, 0, 0, 0, 0, 0];
                ...
                $.each(data.range.success, function(key, invoices) {
                    $('#billing-invoices-table > tbody').prepend([
                        '<tr>',
                            ...
                            '<td><a href="<?= APP::Module('Routing')->root ?>admin/billing/invoices?filters=' + data.range.new[key][3] + '" target="_blank">' + new Intl.NumberFormat().format(data.range.new[key][1]) + '</a></td>',
                            '<td><a href="<?= APP::Module('Routing')->root ?>admin/billing/invoices?filters=' + data.range.processed[key][3] + '" target="_blank">' + new Intl.NumberFormat().format(data.range.processed[key][1]) + '</a></td>',
                            '<td><a href="<?= APP::Module('Routing')->root ?>admin/billing/invoices?filters=' + data.range.revoked[key][3] + '" target="_blank">' + new Intl.NumberFormat().format(data.range.revoked[key][1]) + '</a></td>',
                            '<td><a href="<?= APP::Module('Routing')->root ?>admin/billing/invoices?filters=' + data.range.success[key][3] + '" target="_blank">' + new Intl.NumberFormat().format(data.range.success[key][1]) + '</a></td>',
                            '<td><a href="<?= APP::Module('Routing')->root ?>admin/billing/invoices?filters=' + data.range.access[key][3] + '" target="_blank">' + new Intl.NumberFormat().format(data.range.access[key][1]) + '</a></td>',
                            '<td>' + new Intl.NumberFormat().format(invoices[2]) + ' руб.</td>',
                        '</tr>'
                    ].join(''));
                    ...
                    billing_invoices[4] += data.range.access[key][1];
                    billing_invoices[5] += invoices[2];
                });
                
                $('#billing-invoices-table .total .t_new a').html(new Intl.NumberFormat().format(billing_invoices[0]));
                $('#billing-invoices-table .total .t_processed a').html(new Intl.NumberFormat().format(billing_invoices[1]));
                $('#billing-invoices-table .total .t_revoked a').html(new Intl.NumberFormat().format(billing_invoices[2]));
                $('#billing-invoices-table .total .t_success a').html(new Intl.NumberFormat().format(billing_invoices[3]));
                $('#billing-invoices-table .total .t_access a').html(new Intl.NumberFormat().format(billing_invoices[4]));
                $('#billing-invoices-table .total .t_profit').html(new Intl.NumberFormat().format(billing_invoices[5]) + ' руб.');
            } 
        });
    }

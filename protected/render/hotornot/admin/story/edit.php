    ...
    <body data-ma-header="teal">
        ...
        <section id="main">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                        <form id="update-story" class="form-horizontal" role="form">
                            <input type="hidden" name="id" value="<?= APP::Module('Routing')->get['story_id_hash'] ?>">
                            ...
                            <div class="card-body card-padding">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fg-line">
                                            <textarea name="story" id="story" class="form-control"><?= $data['story']['story'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fg-line">
                                            <textarea name="description" id="description" class="form-control"><?= $data['story']['description'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#email').val('<?= $data['story']['email'] ?>');
                $('#name').val('<?= $data['story']['name'] ?>');
                $('#city').val('<?= $data['story']['city'] ?>');
                $('#caption').val('<?= $data['story']['caption'] ?>');
                
                $('#update-story').submit(function(event) {
                    var form = this;
                    ...
                    $.ajax({
                        type: 'post',
                        url: '<?= APP::Module('Routing')->root ?>admin/hotornot/story/api/update.json',
                        data: $(form).serialize(),
                        success: function(result) {
                            switch(result.status) {
                                case 'success':
                                    swal({
                                        title: 'Done',
                                        text: 'Story "' + name.val() + '" has been updated',
                                        type: 'success',
                            ...
                            $('#update-story').find('[type="submit"]').html('Save changes').attr('disabled', false);
                        }
                    });
                });
            });

...
<!--[if IE 9 ]><html class="ie9"><![endif]-->
    <head>
        ...
        <link href="<?= APP::Module('Routing')->root ?>public/modules/hotornot/story/rules.css" rel="stylesheet">

        <style>
            #story-table-header .actionBar .actions > button {
                display: none;
            }
        </style>
    ...
    <body data-ma-header="teal">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                            ...
                            <ul class="actions">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a href="<?= APP::Module('Routing')->root ?>admin/hotornot/story/add">Add story</a></li>
                        ...
                        <div class="card-body">
                            <table class="table table-hover table-vmiddle" id="story-table">
                                <thead>
                                    <tr>
        ...
        <script src="<?= APP::Module('Routing')->root ?>public/modules/hotornot/story/rules.js"></script> 
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '#search_results_actions a', function () {
                    ...
                    swal({
                        ...
                        if (isConfirm) {
                            $.post('<?= APP::Module('Routing')->root ?>admin/hotornot/story/api/action.json', {
                                action: action,
                                rules: $('#search').val()
                            }, function() { 
                                story_table.bootgrid('reload', true);
                ...
                $(document).on('click', '#render-table', function () {
                    $('#story-table').bootgrid('reload');
                });
                
                var story_table = $("#story-table").bootgrid({
                    ...
                    ajax: true,
                    url: '<?= APP::Module('Routing')->root ?>admin/hotornot/story/api/search.json',
                    css: {
                        icon: 'zmdi icon',
                    ...
                    formatters: {
                        actions: function(column, row) {
                            return  '<a target="_blank" href="<?= APP::Module('Routing')->root ?>admin/hotornot/story/edit/' + row.story_id_token + '" class="btn btn-sm btn-default btn-icon waves-effect waves-circle"><span class="zmdi zmdi-edit"></span></a> ' + 
                                    '<a href="javascript:void(0)" class="btn btn-sm btn-default btn-icon waves-effect waves-circle remove-cost" data-story-id="' + row.id + '"><span class="zmdi zmdi-delete"></span></a>';
                        }
                    }
                }).on('loaded.rs.jquery.bootgrid', function () {
                    story_table.find('.remove-cost').on('click', function (e) {
                        ...
                        swal({
                            ...
                            if (isConfirm) {
                                $.post('<?= APP::Module('Routing')->root ?>admin/hotornot/story/api/remove.json', {
                                    id: story_id
                                }, function() { 
                                    story_table.bootgrid('reload', true);

    ...
    $(document).on('click', '#tab-nav-<?= $data['hash'] ?> > a, #refresh-tunnels',function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#tunnels-list').html([
                    ...
                    '<div class="row m-b-20">',
                        '<div class="col-md-6 media-body ns-item">',
                            ...
                            '<h3 class="c-black">' + new Intl.NumberFormat().format(data.states.active) + '</h3>',
                        '</div>',
                        '<div class="col-md-6 media-body ns-item">',
                            '<small class="c-black">Не проходят туннели</small>',
                            '<h3 class="c-black">' + new Intl.NumberFormat().format(data.states.inactive) + '</h3>',
                        '</div>',
                    '</div>',
                ...
                $.each(data.tunnels.static, function(id, tunnel) {
                    ...
                    $('#static-tunnels-table > tbody').prepend([
                        '<tr>',
                            ...
                            '<td><a href="<?= APP::Module('Routing')->root ?>admin/users?filters=' + tunnel.subscribers.active[1] + '" target="_blank">' + new Intl.NumberFormat().format(tunnel.subscribers.active[0]) + '</a></td>',
                            '<td><a href="<?= APP::Module('Routing')->root ?>admin/users?filters=' + tunnel.subscribers.total[1] + '" target="_blank">' + new Intl.NumberFormat().format(tunnel.subscribers.total[0]) + '</a></td>',
                        '</tr>'
                    ].join(''));
                ...
                $('.static_tunnels_active_total').html(new Intl.NumberFormat().format(c_static[0]));
                $('.static_tunnels_total').html(new Intl.NumberFormat().format(c_static[1]));
                ...
                $('.dynamic_tunnels_active_total').html(new Intl.NumberFormat().format(c_dynamic[0]));
                $('.dynamic_tunnels_total').html(new Intl.NumberFormat().format(c_dynamic[1]));
            }
        });
    });
</script>
                            ...
                            '<div class="btn-group">',
                                '<div class="btn-group">',
                                    ...
                                    '<ul class="dropdown-menu" role="menu">',
                                        ...
                                        '<li><a class="add_trigger_rule" data-logic="intersect" data-method="user_tunnel_tag" href="javascript:void(0)">Тег подписки на туннель</a></li>',
                                        ...
                                        '<li><a class="add_trigger_rule" data-logic="intersect" data-method="user_tunnels" href="javascript:void(0)">Участие в туннеле</a></li>',
                                        ...
                                        '<li><a class="add_trigger_rule" data-logic="intersect" data-method="tunnels_queue" href="javascript:void(0)">Туннели в очереди</a></li>',
                                        '<li><a class="add_trigger_rule" data-logic="intersect" data-method="tunnels_type" href="javascript:void(0)">Прохождение туннеля (по типам)</a></li>',
                                    '</ul>',
                                '</div>',
                                '<div class="btn-group">',
                                    ...
                                    '<ul class="dropdown-menu" role="menu">',
                                        ...
                                        '<li><a class="add_trigger_rule" data-logic="merge" data-method="user_tunnel_tag" href="javascript:void(0)">Тег подписки на туннель</a></li>',
                                        ...
                                        '<li><a class="add_trigger_rule" data-logic="merge" data-method="user_tunnels" href="javascript:void(0)">Участие в туннеле</a></li>',
                                        '<li><a class="add_trigger_rule" data-logic="merge" data-method="user_sale" href="javascript:void(0)">Доступность распродажи</a></li>',
                                        '<li><a class="add_trigger_rule" data-logic="merge" data-method="tunnels_queue" href="javascript:void(0)">Туннели в очереди</a></li>',
                                        '<li><a class="add_trigger_rule" data-logic="merge" data-method="tunnels_type" href="javascript:void(0)">Прохождение туннеля (по типам)</a></li>',
                                    '</ul>',
                                '</div>',
                            '</div>',
                        '</td>',
        ...
        render_rule: function(rule, holder) {
            ...
            switch(rule.method) {
                case 'tunnels_type':
                        ...
                        '<table>',
                            '<tr>',
                                '<td style="width: 125px">',
                                    '<select id="tunnels_users_logic_' + elid + '" class="selectpicker" data-id="logic">',
                                        '<option value="IN">подписан</option>',
                                        '<option value="NOT IN">не подписан</option>',
                                    '</select>',
                                '</td>',
                                '<td style="width: 125px">',
                                    '<select id="tunnels_users_type_' + elid + '" class="selectpicker" data-id="type">',
                    ...
                    $('body').on('change', $('#tunnels_users_logic_' + elid), function(){$target_rules.val($.toJSON(methods.render_value($('#trigger_rules_editor > .trigger_children > .trigger_rule'))))});
                    if (rule.settings.logic !== undefined) $('#tunnels_users_logic_' + elid).val(rule.settings.logic);
                ...
                case 'tunnels_queue':
                    $('.trigger_settings', $trigger_rule_item).append([
                        '<table>',
                            '<tr>Туннели в очереди</tr>',
                            '<tr>',
                                '<td style="width: 125px">',
                                    '<select id="user_tunnels_queue_logic_' + elid + '" class="selectpicker" data-id="logic">',
                                        '<option value="IN">есть</option>',
                                        '<option value="NOT IN">нет</option>',
                                    '</select>',
                                '</td>',
                            '</tr>',
                        '</table>'
                    ...
                    $('#user_tunnels_queue_logic_' + elid)
                    .selectpicker({
                        width: '190px',
                        liveSearch: false
                    });
                ...
                case 'user_tunnels':
                    $('.trigger_settings', $trigger_rule_item).append([
                        '<table>',
                            '<tr>',
                                '<td>',
                                    '<select data-id="logic" id="user_tunnels_logic_' + elid + '" title="Выберите значение...">',
                                ...
                                '</td>',
                                '<td>',
                                    '<input id="user_tunnels_' + elid + '" type="text" data-id="value">',
                                '<td>',
                            '</tr>',
                        '</table>'
                    ].join(''));

                    if (rule.settings.value !== undefined) $('#user_tunnels_' + elid).val(rule.settings.value);
                    $(document).on('change', $('#user_tunnels_' + elid), function(){$target_rules.val($.toJSON(methods.render_value($('#trigger_rules_editor > .trigger_children > .trigger_rule'))))});
                    $('#user_tunnels_' + elid).TunnelSelector({'url':settings.url});
                    
                    if (rule.settings.logic !== undefined) $('#user_tunnels_logic_' + elid).val(rule.settings.logic);
                    $(document).on('change', $('#user_tunnels_logic_' + elid), function(){$target_rules.val($.toJSON(methods.render_value($('#trigger_rules_editor > .trigger_children > .trigger_rule'))))});
                    break;
                /*
                case 'product_availability':
                    $('.trigger_settings', $trigger_rule_item).append([
                ...
                */
                ...
                case 'user_tunnel_tag': 
                    $('.trigger_settings', $trigger_rule_item).append([
                        '<h4>Тег подписки на туннель</h4>',
                        '<table>',
                            '<tr>',
                                '<td style="width: 160px; padding-right: 10px;">',
                                    '<select data-id="mode" class="form-control">',
                                        '<option value="exist">есть тег</option>',
                                        //'<option value="not_exist">нет тега</option>',
                                    '</select>',
                                '</td>',
                                '<td>',
                            ...
                            '<tr>',
                                '<td style="padding-right: 10px;">',
                                    '<br><input data-id="label_id" type="text" class="form-control" placeholder="наименование">',
                                '</td>',
                                '<td>',
                                    '<br><input data-id="token" type="text" class="form-control" placeholder="признак">',
                                '</td>',
                            '</tr>',
                        '</table>',
                        '<div class="panel-group accordion" id="accordion' + elid + '">',
                            '<div class="panel">',
                                ...
                                '<div class="panel-collapse collapse" id="add_settings' + elid + '">',
                                    '<div class="panel-body">',
                                        'Таймаут',
                                        '<input data-id="timeout-value" value="0" type="text" class="form-control" style="width: 80px; display: inline-block;">',
                                        '<select data-id="timeout-mode" class="form-control">',
                                            '<option value="min">мин.</option>',
                                            '<option value="hours">час.</option>',
                                            '<option value="days">дн.</option>',

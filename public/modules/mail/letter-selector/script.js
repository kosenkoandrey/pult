    ...
    var methods = {
        init: function(options) { 
            ...
            if ((letters_manager.val() !== '') && (letters_manager.val() !== '0')) {
                ...
                '<div class="btn-group">',
                    '<a href="javascript:void(0)" target="_blank" class="letter-preview btn btn-default disabled"><span class="glyphicon glyphicon-envelope"></span></a>',
                ...
                '<div class="btn-group">',
                    '<a href="javascript:void(0)" target="_blank" class="letter-preview btn btn-default disabled"><span class="glyphicon glyphicon-envelope"></span></a>',
                    '<a href="javascript:void(0)" class="letter-subject btn btn-default">' + settings.label + '</a>',
                '</div>'
                ].join(''));
            }
                    ...
                    '<div id="letters-panel" class="panel panel-default" data-target="#letters-panel">',
                        ...
                        '<div class="panel-footer">',
                            '<div id="groups-tree" class="btn-group">',
                                '<a href="javascript:void(0)" class="btn btn-default btn-sm active disabled">/</a>',
                            '</div>',
                        '</div>', 
                    '</div>',
                '</div>'
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data.path, function(index, item) {
                        if (item[0] == group) {
                            $('#groups-tree').append('<a href="javascript:void(0)" class="btn btn-default btn-sm active disabled">' + item[1] + '</a>');
                        } else {
                            $('#groups-tree').append('<a href="javascript:void(0)" data-group="' + item[0] + '" class="btn btn-default btn-sm">' + item[1] + '</a>');
                        }
                    });

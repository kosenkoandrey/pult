            ...
            switch (t) {
                ...
                case 'text-block' :
                    ...
                    case 'video':
                        ...
                        var img_link = img.parent('a.link');
                        $('#youtube-url').val(img_link.attr('href'));
                        ...
                        $('#imageid').val(imageid);
                        $('#youtube-image-url').val(img.attr('src'));
                        $('#youtube-image-url').data('id', imageid);

                        $('#youtube-image-w').val(img.css('width'));
                        $('#youtube-image-h').val(img.css('height'));
                ...
                case 'image':
                    ...
                    var img_link = img.parent('a.link');
                    $('#image-link-url').val(img_link.attr('href'));
                ...
                case 'imgtxtincol':
                    ...
                    imgs.each(function (i) {
                        ...
                        img.bind('click', function () {
                            ...
                            var img_link = $(this).parent('a.link');
                            $('#image-link-url').val(img_link.attr('href'));
...
function downloadLayoutSrc() {
    ...
    clone.find('td#primary').html(header + body + footer);
...
function initEmailBuilder() {
    ...
    $('#youtube-change-image').on('click', function (e) {
        var id = $('#youtube-image-url').data('id');
        $('#' + id).attr('src', $('#youtube-image-url').val());
        $('#' + id).attr('width', $('#youtube-image-w').val());
        $('#' + id).attr('height', $('#youtube-image-h').val());

        var link = $('#' + id).parent('a.link');
        link.attr('href', $('#youtube-image-link-url').val());
    ...
    $('#closeSideBarnav').click(function(e){
        e.preventDefault();
        $('#sidebar-opzioni').hide();
    });
    ...
    $(document).on('change', '.social-check', function (e) {
        ...
        var name = $(this).attr('name');
        if ($(this).is(':checked')) {
            $('#' + $('#path').val()).find($('#selector').val() + ' a.' + name).show().removeClass('remove');
        } else {
            $('#' + $('#path').val()).find($('#selector').val() + ' a.' + name).hide().addClass('remove');
        }
    ...
    $("#save").click(function () {
        ...
        $.post(path + '/save.php', {option: 'save', id: save.data('id'), html: $('#download').val()}, function (data) {
            ...
            $('#messagefromphp2').html("<a style='color:#000000' target=_blank href=" + data + ">" + data + "</a>");
        }, 'html');
    ...
    $('#generayoutube').click(function () {
        $.post(path + '/videotoimg.php', { youtubevideo: $('#youtube-image-link-url').val()}, function (data) {

            //oppure se il php nn va metodo tutto in js
            linkyoutube=$('#youtube-image-link-url').val();
            linkyoutube = linkyoutube.replace("https://www.youtube.com/watch?v=", "");
            linkyoutube = linkyoutube.replace("https://www.youtube.com/watch?v=/", "");
            $('#youtube-image-url').val(scrippath+"images/"+linkyoutube+".jpg");

            var img = self.find('img');
            var img_link = img.parent('a.link');
            img_link.attr('href')=$('#youtube-image-link-url').val();
...
$(document).ready(function() {
    $(document).on('click', '.em_menu', function (e) {

        var group = $(this).data('group');
       // if (!$("#sidebar-opzioni").is(':visible')) {
            $.post(path + '/select', {group: group}, function (responce) {
                $('#estRows').show().html(responce);
                $(".sidebar-nav .lyrow").draggable({connectToSortable: ".column", helper: 'clone',
                    stop: function (e, t) {
                        handleObjects();
                        $(".demo .column").sortable({opacity: .35, connectWith: ".column"});
        ...
        //}
        $("#sidebar-opzioni").toggle();
        //$("#sidebar-opzioni").height($( window ).height()+100);
        $("#sidebar-opzioni").height(800);
        $("#page-content-wrapper").toggleClass("active3");
